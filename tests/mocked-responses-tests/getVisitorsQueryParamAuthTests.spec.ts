...
describe('[Mocked response] Get Visitors', () => {
  ...
  const client = new FingerprintJsServerApiClient({region: Region.EU, apiToken: authToken, authenticationMode: AuthenticationMode.QueryParameter});
  ...
  test('with request_id filter', async () => {
    (fetch as unknown as jest.Mock).mockReturnValue(Promise.resolve(new Response(JSON.stringify(visitorsResponseWithRequestId))));
  ...
  test('with request_id and linked_id filter', async () => {
    (fetch as unknown as jest.Mock).mockReturnValue(Promise.resolve(new Response(JSON.stringify(visitorsResponseWithRequestIdLinkedId))));
  ...
  test('with linked_id and limit filter', async () => {
    (fetch as unknown as jest.Mock).mockReturnValue(Promise.resolve(new Response(JSON.stringify(visitorsResponseWithLinkedIdLimit))));
  ...
  test('with limit and before', async () => {
    (fetch as unknown as jest.Mock).mockReturnValue(Promise.resolve(new Response(JSON.stringify(visitorsWithLimitBefore))));
